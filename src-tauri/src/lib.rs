...
use std::{env, fs, io::{BufReader, Read}, path::{Path, PathBuf}, process::{Child, Command, Stdio}, sync::{atomic::{AtomicBool, Ordering}, Arc, Mutex}, thread};
...
fn read_cli_records<R: Read>(reader: R, mut on_record: impl FnMut(String)) {
  ...
  loop {
    ...
    for byte in &chunk[..read] {
      if *byte == b'\n' || *byte == b'\r' {
        if !record.is_empty() {
          on_record(String::from_utf8_lossy(&record).into_owned());
          record.clear();
        }
      } else {
        record.push(*byte);
  ...
  if !record.is_empty() { on_record(String::from_utf8_lossy(&record).into_owned()); }
}
...
async fn process_video(app: AppHandle, controller: State<'_, ProcessController>, request: ProcessRequest) -> Result<ProcessResult, String> {
  ...
  tauri::async_runtime::spawn_blocking(move || {
    ...
    if let Some(pipe) = stdout { let app = app.clone(); let job_id = request.job_id.clone(); thread::spawn(move || read_cli_records(pipe, |text| { let _ = app.emit("cli-output", CliOutput { job_id: job_id.clone(), stream: "stdout", text }); })); }
    if let Some(pipe) = stderr { let app = app.clone(); let job_id = request.job_id.clone(); thread::spawn(move || read_cli_records(pipe, |text| { let _ = app.emit("cli-output", CliOutput { job_id: job_id.clone(), stream: "stderr", text }); })); }
...
#[cfg(test)]
mod tests {
  use super::read_cli_records;

  #[test]
  fn reads_newline_and_carriage_return_progress_records() {
    let input = b"Loaded\n[----------] 0% (1/240)\r[#---------] 10% (24/240)\rDone\r\n";
    let mut records = Vec::new();
    read_cli_records(&input[..], |record| records.push(record));
    assert_eq!(records, ["Loaded", "[----------] 0% (1/240)", "[#---------] 10% (24/240)", "Done"]);
  }
}
